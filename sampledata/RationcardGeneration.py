...
text_width = draw.textbbox((0, 0), "CIVIL SUPPLIES & CONSUMER PROTECTION DEPARTMENT", font=font_bold)[2]
...
# Replace XXXX with Real Data
user_info = {
    "Family Head Name": "Vinayak Mahadev",
    "Father's/Husband's Name": "Siva",
    "Date of Birth": "01-01-1970",
    "Address": "Chennai, Tamil Nadu, India",
    "PHAA": "PHAA",
    "Card Number": "587654321012"
}

# Details and values
details = ["Family Head Name", "Father's/Husband's Name", "Date of Birth", "Address"]
values = [user_info["Family Head Name"], user_info["Father's/Husband's Name"], 
          user_info["Date of Birth"], user_info["Address"]]

# Find the longest text width to align all `:` properly
...
draw.text((box1_x + 15, box1_y + 10), user_info["PHAA"], font=font_bold, fill="black")
...
card_number = user_info["Card Number"]
...
# ✅ Load and Paste Profile Picture (if exists)
profile_image_path = r"c:\\Users\\swarn\\Downloads\\man.webp"  # Update this path

if os.path.exists(profile_image_path):
    profile_img = Image.open(profile_image_path).resize((profile_width, profile_height))
    card.paste(profile_img, (profile_x, profile_y))  # Place over the placeholder
else:
    draw.text((profile_x + 40, profile_y + 50), "Photo", font=font_bold, fill="black")

# Save and Show the Final Smart Ration Card
output_folder = "generated_smartcards"
os.makedirs(output_folder, exist_ok=True)

output_path = os.path.join(output_folder, f"{user_info['Family Head Name']}_smart_card.png")
card.save(output_path)
card.show()

print(f"✅ Smart Ration Card saved at: {output_path}")
